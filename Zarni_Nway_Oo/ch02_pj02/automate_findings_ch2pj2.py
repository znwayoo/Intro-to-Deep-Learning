...
class ClassificationModelTrainer:
    ...
    def load_and_prepare_data(self):
        ...
        self.y = df['SR_Name'].values 
        self.X = df.drop(columns=['SR_Name']).values
    ...
    def create_executive_summary(self, all_results, detailed_reports):
        ...
        if detailed_reports['train']:
            ...
            train_detailed.to_csv('results/detailed_classification_report_train.csv', index=False)
        
        if detailed_reports['test']:
            test_detailed = pd.concat(detailed_reports['test'], ignore_index=True)
            test_detailed.to_csv('results/detailed_classification_report_test.csv', index=False)
        ...
        print(f"- Main summary: results/executive_summary.csv")
        if detailed_reports['train']:
            print(f"- Detailed train reports: results/detailed_classification_report_train.csv")
        if detailed_reports['test']:
            print(f"- Detailed test reports: results/detailed_classification_report_test.csv")
        if len(summary_df) > 0:
