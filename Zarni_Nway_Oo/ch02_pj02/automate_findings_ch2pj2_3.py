...
class ClassificationModelTrainer:
    ...
    def load_and_prepare_data(self):
        ...
        self.X = dp.preprocess_textinput_tfidf(df, 'name')
    ...
    def train_single_model(self, model_config, model_name):
        ...
        # Create directory
        save_dir = os.path.join('results3', model_name)
    ...
    def create_executive_summary(self, all_results, detailed_reports):
        ...
        summary_df.to_csv('results3/executive_summary.csv', index=False)
        
        # Detailed classification reports
        if detailed_reports['train']:
            train_detailed = pd.concat(detailed_reports['train'], ignore_index=True)
            train_detailed.to_csv('results3/detailed_classification_report_train.csv', index=False)
        
        if detailed_reports['test']:
            test_detailed = pd.concat(detailed_reports['test'], ignore_index=True)
            test_detailed.to_csv('results3/detailed_classification_report_test.csv', index=False)
        ...
        print(f"- Main summary: results3/executive_summary.csv")
        if detailed_reports['train']:
            print(f"- Detailed train reports: results3/detailed_classification_report_train.csv")
        if detailed_reports['test']:
            print(f"- Detailed test reports: results3/detailed_classification_report_test.csv")
        if len(summary_df) > 0:
            print(f"- Performance comparison: results3/performance_comparison.png")
    ...
    def create_performance_comparison(self, summary_df):
        ...
        plt.tight_layout()
        plt.savefig('results3/performance_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
